...
inline ea_t idalib_find_text(ea_t start_ea, const char *text) {
  return find_text(start_ea, 0, 0, text, SEARCH_DOWN | SEARCH_NEXT);
}

inline ea_t idalib_find_imm(ea_t start_ea, uint32 imm) {
  return find_imm(start_ea, SEARCH_DOWN | SEARCH_NEXT, imm, nullptr);
}

inline ea_t idalib_find_defined(ea_t start_ea) {
