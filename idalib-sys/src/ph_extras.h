...
inline std::int32_t idalib_ph_id(const processor_t *ph) {
  return ph->id;
}

inline rust::String idalib_ph_short_name(const processor_t *ph) {
  auto name = ph->psnames[const_cast<processor_t *>(ph)->get_proc_index()];
  return rust::String(name);
}

inline rust::String idalib_ph_long_name(const processor_t *ph) {
